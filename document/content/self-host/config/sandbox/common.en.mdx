...
Use [agent-sandbox-proxy.yml](/deploy/sandbox_deploy/agent-proxy.yml) as a reference and add the service to your yml file. Expose the proxy port that browsers will connect to, and record the `AGENT_SANDBOX_PROXY_SECRET` value, which you will need in the next step.
...
| `FASTGPT_APP_REQUEST_TIMEOUT_SECS` | `10`                                     | Timeout, in seconds, for proxy requests back to the FastGPT main service. The sample yml sets it to `60` to cover sandbox cold starts. |
...
### Common resource limit variables

Configure the following variables in `fastgpt-app` and `fastgpt-pro` when you need to adjust common resource limits:
...
| `AGENT_SANDBOX_ENTRYPOINT_TIMEOUT_SECONDS` | `30`       | Timeout for Agent Sandbox entrypoint execution, in seconds. Allowed range: `1` to `600`.                                          |
