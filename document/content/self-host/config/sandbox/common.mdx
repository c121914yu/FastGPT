...
可以参考 [agent-sandbox-proxy.yml](/deploy/sandbox_deploy/agent-proxy.yml)，将 service 加到 yml 文件里。并开放 proxy 的浏览器访问端口。并记录 `AGENT_SANDBOX_PROXY_SECRET` 环境变量，下一步需要使用。
...
| `FASTGPT_APP_REQUEST_TIMEOUT_SECS` | `10`                                     | 代理回源 FastGPT 主服务的请求超时时间，单位秒；示例 yml 为覆盖冷启动场景显式配置为 `60`。 |
...
`fastgpt-app` 和 `fastgpt-pro` 中你可以通过以下变量控制常用资源限制：

| 变量                                       | 默认值     | 说明                                                                             |
| ------------------------------------------ | ---------- | -------------------------------------------------------------------------------- |
| `AGENT_SANDBOX_DISK_MB`                    | `1024`     | Agent 沙盒磁盘大小基准，单位 MB；会影响冷归档包、Skill 包和 IDE 单文件大小限制。 |
| `AGENT_SANDBOX_MAX_EDIT_DEBUG`             | `100`      | Agent 编辑/调试沙盒数量限制。                                                    |
| `AGENT_SANDBOX_ENTRYPOINT_TIMEOUT_SECONDS` | `30`       | Agent 沙盒 entrypoint 执行超时时间，单位秒，允许范围为 `1` 到 `600`。            |
