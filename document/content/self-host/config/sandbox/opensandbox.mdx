...
参考 [opensandbox.yml](/deploy/sandbox_deploy/opensandbox.yml)，将 `fastgpt-opensandbox-server`、`fastgpt-volume-manager`、预拉取镜像和 `opensandbox-config` 加入当前 FastGPT 部署的 `docker-compose.yml`，并放到 FastGPT App 所在的 `app` network 中；不需要对外暴露 OpenSandbox 或 Volume Manager 端口。这里的不对外暴露只针对 OpenSandbox 和 Volume Manager，Agent Sandbox Proxy 的浏览器访问端口仍需按[沙盒通用配置](./common)单独部署并开放。
...
如果服务器配置了 `HTTP_PROXY` / `HTTPS_PROXY`，建议给 OpenSandbox Server 和 Volume Manager 补充 `NO_PROXY` / `no_proxy`，至少包含 `localhost,127.0.0.1,127.0.0.0/8,fastgpt-opensandbox-server,fastgpt-volume-manager,fastgpt-app,host.docker.internal,host.orb.internal,docker.orb.internal,*.orb.internal,*.orb.local`，避免内部服务调用被代理劫持。OrbStack/Docker 可能自动注入包含 IPv6 CIDR 的 `NO_PROXY`，OpenSandbox 依赖的 httpx 可能把未加方括号的 IPv6 CIDR 误解析成 URL 端口，导致启动失败；遇到该问题时应显式覆盖 `NO_PROXY`。
...
注意：OpenSandbox 持久卷只读取 `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_URL` 和 `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN`。不要使用旧版 `AGENT_SANDBOX_VOLUME_MANAGER_URL`、`AGENT_SANDBOX_VOLUME_MANAGER_TOKEN` 或 `AGENT_SANDBOX_ENABLE_VOLUME`。
